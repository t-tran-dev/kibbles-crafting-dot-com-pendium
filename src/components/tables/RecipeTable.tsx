  ...
  const handleFilterList = () => {
    const rows = originalRows.filter((row) => {
      if (
        ...
        (school == "all" ||
          row.school.toLowerCase().includes(school.toLowerCase())) &&
    ...
    <Box sx={{ width: "100%" }}>
      ...
      <div style={viewState == "projects" ? { display: "none" } : {}}>
        <FormControlLabel
          style={{ color: "green" }}
          control={
            <Switch checked={craftableOnly} onChange={handleChangeCraftable} />
          }
          label="Only show craftable"
        />
        <FormControlLabel
          control={
            <Switch checked={recipeOnly} onChange={handleChangeRecipe} />
          }
...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  let addToInventoryEnabled = !(
    props.viewState == "items" || props.viewState == "projects"
  );
  let addToInventoryText =
    props.viewState == "items"
      ? "Add To Inventory"
      : "Complete Project and Add to Inventory";
  return (
    <TableHead>
      <TableRow>
